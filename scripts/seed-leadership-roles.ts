...
import { leadershipRoles, ministryTeams } from '../src/lib/schema';
...
async function seedLeadershipRoles() {
  try {
    console.log('🌱 Seeding leadership roles...\n');

    // General Board positions (single occupant each except MVT and La Reunion)
    const boardRoles = [
    ...
    for (const role of boardRoles) {
      ...
      console.log(`✅ Added board role: ${role.name} (max: ${role.max})`);
    }
    ...
    console.log('✅ Added Elder role (max: 14)');
    ...
    for (const ministry of ministries) {
      if (ministry.contactPerson && ministry.contactPerson.trim()) {
        await db.insert(leadershipRoles).values({
          ...
          maxPositions: 1, // Most ministries have 1 leader, can be updated manually if needed
          sortOrder: count,
          isActive: true,
        });
        console.log(`✅ Added ministry leader role: ${ministry.contactHeading || ministry.name + ' Leader'}`);
        count++;
      }
    }

    console.log(`\n🎉 Successfully seeded ${boardRoles.length + 2 + count} leadership roles!`);
    console.log(`   - ${boardRoles.length} board positions`);
    console.log(`   - 1 elder role (14 positions)`);
    console.log(`   - 1 deacon role (36 positions)`);
    console.log(`   - ${count} ministry leader roles`);
